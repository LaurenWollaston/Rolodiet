  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 style={{display:'flex',flexDirection:'row',justifyContent:'center',color:'white'}}>{selectedCard.name}</h2>
        </div>
        <p style={{marginTop:"-12px",marginBottom:"-20px"}}>by</p>
        <h4 style={{color:'#e3e38f'}}>{selectedCard.author}</h4>
        <p style={{color:'rgb(211 119 119)'}}>{selectedCard.ingredients}</p>
        <p style={{ whiteSpace: "pre-line" }}>{selectedCard.text}</p>
